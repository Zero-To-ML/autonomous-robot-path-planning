...
class DFSSearch:
    def __init__(self, start, goal, grid, path):
        ...
        self.grid = grid
        self.path = path
        self.stack = [start]  # Stack for DFS
        ...
        self.depth = 0  # Current exploration depth
        
    def run_dfs(self):
        """Run the DFS algorithm until the goal is found or the stack is empty"""
        explored_count = 0
        max_depth = 1000  # Limit depth to prevent excessive exploration
        
        while self.stack:
            current_pos = self.stack.pop()  # Pop from stack (LIFO)
...
# Initialize DFS
dfs = DFSSearch(start, goal, grid, path)
...
# First visualization - DFS exploration with start/goal markers and all roads
...
explored_mask = path > -1000
plt.imshow(np.where(explored_mask, path, np.nan), cmap='plasma', alpha=0.7)
...
plt.title('DFS Search Exploration')
plt.legend(loc='upper right')
plt.colorbar(label='Depth of Exploration')
plt.tight_layout()
plt.show()
